...
t_color	light_angle_plane(t_scene *scene, t_ray *ray)
{
	t_plane		*plane;
	float		dot_prod;
	t_vec3		light;
	...
	if (dot_prod > 0)
		return (XCOL_BLK);
	return (color_shift(plane->color, XCOL_BLK, (dot_prod)));
}

t_color	light_angle_sphere(t_scene *scene, t_ray *ray)
{
	t_sphere	*sphere;
	float		dot_prod;
	t_vec3		normal;
...
t_color light_angle(t_scene *scene, t_ray *ray, int light_color)
{
	if (ray->object_type == SEL_SPHERE)
		return (light_angle_sphere(scene, ray));
	//	planes not working yet
	/*if (ray->object_type == SEL_PLANE)*/
		/*return (light_angle_plane(scene, ray));*/
	else
		return (light_color);
}
